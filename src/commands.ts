...
export async function containerizeHandler(context: ExtensionContext) {
    ...
    const pubTask = new vscode.Task(
        ...
        'Publish',
        'dotnet',
        new vscode.ShellExecution(`dotnet publish ${projectPath} --os ${(await inputs).os} --arch ${(await inputs).architecture} /p:ContainerBaseImage=${(await inputs).containerBaseImage} /p:ContainerImageTag=${(await inputs).tag} -c Release /t:PublishContainer /p:ContainerRepository=${(await inputs).imageName}`)
    );
